...
export default async function ContactedPage({ searchParams }: ContactedPageProps) {
  const params = await searchParams;
  const page = parseInt(params.page || "1", 10);

  const [leadsResult, categoriesResult] = await Promise.all([
    getLeads(
      { status: "contacted", isDeleted: false, category: params.category, priority: params.priority, search: params.search },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Contacted Leads</h1>
          <p className="text-slate-500">
            {pagination?.total || 0} leads you have contacted
          </p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                   ...
                   <th className="px-4 py-3 text-left text-sm font-semibold text-slate-700">Contacted</th>
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-3">
                     ...
                     <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600">
                        {lead.contactedAt ? formatDate(lead.contactedAt) : "-"}
                     </td>
                   </tr>
                ))}
                {leads.length === 0 && (
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {pagination.page > 1 && (
              <Link href={`/contacted?${new URLSearchParams({ ...params, page: String(pagination.page - 1) }).toString()}`}>
