...
export default async function DeclinedPage({ searchParams }: DeclinedPageProps) {
  ...
  const [leadsResult, categoriesResult] = await Promise.all([
    getLeads(
      { status: "declined", isDeleted: false, category: params.category, search: params.search },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Declined Leads</h1>
          <p className="text-slate-500">
            {pagination?.total || 0} leads that declined the offer
          </p>
        </div>
        <Link href="/leads/new">
          <Button>
        ...
        <CardContent className="pt-6">
          <form className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input
                name="search"
                placeholder="Search declined leads..."
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {pagination.page > 1 && (
              <Link href={`/declined?${new URLSearchParams({ ...params, page: String(pagination.page - 1) }).toString()}`}>
